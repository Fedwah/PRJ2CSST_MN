...
public class EntityFields<T> {
    ...
    public void generateFields(Class<T> beanClass ) {
        ...
        for(Field f : beanClass.getDeclaredFields()) {
            fd = new FieldDefinition(formatName( f.getName() ),formatLabel( formatField(f.getName())),formatClass(f.toGenericString()), isBasicClass(f.toGenericString()));
            System.out.println( "field add : "+formatField(f.getName())+" | "+fd.label+" | "+fd.class_+" | "+fd.isBasicClass);
    ...
    public void putField(String field,FieldDefinition definition) {
        if(definition.label!=" " && field!=" " && Arrays.binarySearch( fieldsToBan, field)==-1) {
